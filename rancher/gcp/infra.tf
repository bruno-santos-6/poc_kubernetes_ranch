...
# GCP Public Compute Address for quickstart node
resource "google_compute_address" "quickstart_node_address_2" {
  name = "quickstart-node-ipv4-address-2"
}
...
# GCP compute instance for creating a second node workload cluster
resource "google_compute_instance" "quickstart_node_2" {
  depends_on = [
    google_compute_firewall.rancher_fw_allowall,
  ]
  ...
  network_interface {
    network = "default"
    access_config {
      nat_ip = google_compute_address.quickstart_node_address_2.address
    }
  }
  ...
  metadata_startup_script = templatefile(
    ...
    {
      register_command = module.rancher_common.custom_cluster_command
      public_ip        = google_compute_address.quickstart_node_address_2.address
    }
  )
